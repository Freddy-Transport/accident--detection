...
def main() -> int:
    ...
    ensure_dir(work_dir)
    try:
        cmd = build_cmd('test', config_path, work_dir, args.checkpoint)
    except Exception as exc:
        print(json.dumps({'status': 'blocked', 'reason': str(exc)}, ensure_ascii=False, indent=2))
        return 2

    print(json.dumps({'status': 'running', 'mode': 'test', 'model': args.model, 'config': config_path, 'work_dir': str(work_dir), 'cmd': cmd}, ensure_ascii=False, indent=2))
    env = os.environ.copy()
